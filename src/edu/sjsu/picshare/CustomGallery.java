...
public class CustomGallery extends Activity {
			...
			ArrayList<String> selectedItems = imageAdapter.getCheckedItems();	
			 for (int i = 0; i < selectedItems.size(); i++) {
		          ...
		          // Create the ParseFile
					ParseFile file = new ParseFile("androidbegin.png", byteArrayImage);
				...
				Toast.makeText(CustomGallery.this, i+1+" Image Uploaded",
			        ...
			        final ParseQuery query = new ParseQuery("ImageUpload");
			 		
			        query.whereEqualTo("ImageName","AndroidBegin Logo");
			        final ArrayList<ParseFile> retrievedImages=new ArrayList<ParseFile>();
			        query.findInBackground(new FindCallback<ParseObject>() {
			            public void done(List<ParseObject> imagesTable, ParseException e) {
			            	ParseFile fileObject;
			                if (e == null) {
			                    Log.d("size", "Retrieved " + imagesTable.size());
			                  
			                    System.out.println(imagesTable.get(0).getObjectId());
									...
									try {
										obj = query.get(imagesTable.get(0).getObjectId());
									

			                    	 fileObject= (ParseFile) obj.get("ImageFile");
			                    	 
			                        byte[] file = fileObject.getData();
			                         
			                         
			             /*     
			                         for(int i=0;i<imagesTable.size();i++)
					                    {
					                    	obj=query.get("ImageFile");
					                    	fileObject=(ParseFile) obj.getParseFile("ImageFile");
					                    	retrievedImages.add(fileObject);
					                    }
			                         
			                         
			                        byte[] file= retrievedImages.get(0).getData();
    ...
    public void viewphoto(byte[] file)
    {
